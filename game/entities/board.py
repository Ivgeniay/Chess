...
class Board(IDrawable):
    ...
    def draw_board(self, surface: pg.Surface) -> None:
        ...
        for row in range(size):
            for col in range(size):
                color: tuple[int, int, int] = WHITE if (
                    row + col) % 2 == 0 else BLACK_SOFT
                rect_pos: tuple[int, int, int, int] = self.get_cell_infos_by_chess_coord(
                    col, row)
    ...
    def ouline_cell(self, surface: pg.Surface, move: Move, color: pg.Color = RED) -> None:
        cell_info: tuple[int, int, int, int] = self.get_cell_info_by_move(move)
    ...
    def get_cell_by_absolut_screen_coord(self, x: int, y: int) -> Move | None:
        ...
        x_relative: int = x - BOARD_OFFSET[0]
        y_relative: int = y - BOARD_OFFSET[1]

        if x_relative < 0 or y_relative < 0:
            return None

        col: int = x_relative // (SQUARE_SIZE // SQUARE_SCALE)
        row: int = y_relative // (SQUARE_SIZE // SQUARE_SCALE)
        row = 7 - row

        if not (0 <= col < 8 and 0 <= row < 8):
            return None

        col_letter: str = chr(ord('a') + col)
    ...
    def get_cell_infos_by_chess_coord(self, x: int, y: int) -> tuple[int, int, int, int]:
        ...
        """
        adjusted_y: int = 7 - y  # Инверсия по вертикали
        x = x * SQUARE_SIZE
